    ...
    <div className="prompt_card">
      <div className="flex justify-between items-start gap-5">
        ...
        onClick={handleProfileClick}>
          <Image src={post.creator.image} width={37} height={37} className='rounded-full 
          object-contain' alt="profile"/>
          <div>
            <h3 className="font-satoshi font-semibold text-gray-900">
              {post.creator.username}</h3>
            <p className="text-inter text-sm text-gray-500">
              {post.creator.email}</p>
          </div>
        </div>
        <div className="copy_btn" onClick={handleCopy}>
          <Image src={copied===post.prompt ? '/assets/icons/tick.svg' : 
          '/assets/icons/copy.svg'} width={12} height={12} alt="copy-btn" />
        </div>
      </div>
      ...
      {session?.user.id === post.creator._id && pathName === '/profile'&&(
        <div className="mt-5 flex-center gap-4 border-t pt-3 border-gray-200">
